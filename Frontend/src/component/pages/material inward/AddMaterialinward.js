...
export default function AddMaterialInward() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        const response = await fetch(`${APP_BASE_PATH}/getPoForInward`); // Replace with your API endpoint
  ...
  const onInward = (id, miid) => {
    ...
    navigate(`/addinward?id=${id}&miid=${miid}`);
  };
                        ...
                        .map((item, index) => {
                          return (
                            ...
                            >
                              ...
                              <TableCell align="center" key={item.u}>
                                {item.suppname}
                              </TableCell>
                              ...
                              <TableCell align="center">
                                {item.status != 2 && (
                                  <Button
                                    onClick={() => onInward(item.id, item.miid)}
                                  >
                                    Inward
                                  </Button>
                                )}
