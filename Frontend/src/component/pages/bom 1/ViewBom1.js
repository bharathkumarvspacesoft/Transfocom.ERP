...
export default function ViewBom1() {
  ...
  const fetchData = async () => {
    setIsLoading(true);
    try {
      const response = await fetch(`${APP_BASE_PATH}/viewbom1/${id}`, {
      ...
      if (jsonData && jsonData.length > 0) {
        setCostingName(jsonData[0].CostingName || "");
      }
  ...
  const handlePrint = () => {
    ...
    <body>
    <u> <p style="padding-top: 3pt;padding-bottom: 1pt;padding-left: 90pt;text-indent: 0pt;text-align: left;">${data[0]?.Capacity || null
        } KVA, ${data[0]?.VoltageRatio || null}V, ${data[0]?.TypeTaping}, ${data[0]?.Matofwind
        },${data[0]?.ConsumerType}, ${data[0]?.Type
        }</u> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;           <u>Dtd. ${data[0]?.CostingDate || null
        }</u></p>
   
            <br/>
   
        <table style="border-collapse:collapse;margin-left:5.5pt;">
              ...
              <div style={{ height: 400, width: "100%" }}>
                ...
                >
                  <TextField
                    ...
                    value={
                      costingName && costingName.trim() !== ""
                        ? costingName
                        : `${data[0]?.Capacity || ""} KVA, ${data[0]?.VoltageRatio || ""
                        }V, ${data[0]?.TypeTaping || ""}, ${data[0]?.Matofwind || ""
                        },${data[0]?.ConsumerType || ""}, ${data[0]?.Type || ""
                        }`
                    }
                  />
                    ...
                    <Table className="tabel">
                      ...
                      <TableBody>
                        {filteredData
                          .filter(
                            (row) =>
                              (row.item_code?.toLowerCase() ?? "").includes(
                                searchQuery.toLowerCase()
                              ) ||
                              (
                                row.material_description?.toLowerCase() ?? ""
                              ).includes(searchQuery.toLowerCase()) ||
                              (row.unit?.toLowerCase() ?? "").includes(
                                searchQuery.toLowerCase()
                              ) ||
                              (
                                row.corresponding_unit?.toLowerCase() ?? ""
                              ).includes(searchQuery.toLowerCase())
                          )
                          .slice(
                            page * rowsPerPage,
